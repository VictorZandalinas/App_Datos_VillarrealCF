...
class FaltasFrontalesReport:
    ...
    def clean_and_filter_data(self):
        ...
        self.df = self.df[
            (self.df['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (Nº)') &
    ...
    def get_all_teams_data(self, jornada_hasta):
        ...
        df_faltas_frontales = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (Nº)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        df_exito_frontales = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (% Éxito)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        df_rematados_frontales = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (% Rematados)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        # Limpiar nombres de equipos en todos los dataframes
        for df in [df_faltas_frontales, df_exito_frontales, df_rematados_frontales]:
        ...
        if 'ID PARTIDO' in df_faltas_frontales.columns:
            faltas_total = df_faltas_frontales.groupby('EQUIPO').agg({
                'VALOR': 'sum',
        ...
        exito_promedio = df_exito_frontales.groupby('EQUIPO').agg({
            'VALOR': 'mean'
        }).reset_index()
        exito_promedio.columns = ['EQUIPO', 'EXITO_PROMEDIO']
        
        rematados_promedio = df_rematados_frontales.groupby('EQUIPO').agg({
            'VALOR': 'mean'
        }).reset_index()
        ...
        combined_data.columns = ['EQUIPO', 'FALTAS_FRONTALES', 'REMATADOS', 'EXITO']
    ...
    def create_visualization(self, equipo, jornada_hasta, figsize=(11.69, 8.27)):
        ...
        # Título principal
        ax_title.text(0.5, 0.8, 'FALTAS OFENSIVAS FRONTALES', 
        ...
        ax_evolution.set_title(f'FALTAS FRONTALES POR PARTIDO - {equipo.upper()}', 
        ...
        ax_ranking.set_title('MEDIA DE FALTAS FRONTALES A FAVOR', 
                            fontsize=12, weight='bold', color='#1e3d59', pad=10)
    ...
    def plot_evolution_chart(self, ax, filtered_df, jornada_hasta):
        ...
        ax.set_ylabel('Nº Faltas Frontales', fontsize=10, weight='bold')
    ...
    def plot_general_stats(self, ax, filtered_df, all_teams_data, equipo):
        ...
        def get_team_metrics(team_name, jornada_hasta):
            ...
            faltas_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (Nº)']
            ...
            exito_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (% Éxito)']
            if len(exito_data) > 0:
            ...
            rematados_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P F. Frontales a favor (% Rematados)']
    ...
    def plot_team_ranking(self, ax, all_teams_data, selected_equipo):
        ...
        sorted_data = all_teams_data.sort_values('FALTAS_FRONTALES', ascending=True)
        ...
        bars = ax.barh(range(len(sorted_data)), sorted_data['FALTAS_FRONTALES'], 
                    color=colors, alpha=0.8, edgecolor='#2c3e50', linewidth=0.5)
        
        # Añadir valores solo para Villarreal y equipo seleccionado
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['FALTAS_FRONTALES'], sorted_data['EQUIPO'])):
        ...
        ax.set_xlabel('Media faltas frontales a favor', fontsize=10, weight='bold')
        
        # Línea promedio de la liga
        promedio_liga = sorted_data['FALTAS_FRONTALES'].mean()
    ...
    def plot_team_scatter(self, ax, all_teams_data, selected_equipo):
        ...
        # Extraer datos
        x_data = all_teams_data['FALTAS_FRONTALES']  # Faltas frontales por partido
