...
class AnalizadorSaquesBanda:
    ...
    def load_data(self, team_filter=None):
        ...
        try:
            self.df_complete = pl.read_parquet(self.data_path)

            # Filtrar por equipo si se especifica
            if team_filter:
                self.team_filter = team_filter
                self.df = self.df_complete.filter(pl.col('Team Name') == team_filter)
            else:
                self.df = self.df_complete.clone()
    ...
    def extract_throw_in_sequences(self, team_name):
        ...
        throw_ins = self.df.filter(
            (pl.col('Team Name') == team_name) &
            (pl.col('Event Name') == 'Pass') &
            (pl.col('Throw in') == 'Sí')
        )
        ...
        # Añadir índice para poder iterar
        throw_ins_with_idx = throw_ins.with_row_index('row_idx')

        for row in throw_ins_with_idx.iter_rows(named=True):
            # --- LECTURA Y CONVERSIÓN DEL ÁNGULO (LÓGICA FINAL Y CORRECTA) ---
            final_angle = None
            try:
                # 1. Leer el ángulo en radianes y convertir a grados
                angle_rad = float(row.get('Angle'))
                angle_deg = np.degrees(angle_rad)

                side = 'Izquierda' if row.get('y', 50) >= 50 else 'Derecha'
            ...
            speed_category = self.calculate_throw_speed(row.get('row_idx'), row.get('timeStamp'))
            ...
            sequence = {
                ...
                'zone_origin': self.get_zone_from_x_coord(row.get('x', 0)),
    ...
    def calculate_throw_speed(self, current_idx, current_timestamp):
        ...
        """
        if current_timestamp is None or current_idx is None:
            return 'Desconocido'

        # Convertir timestamp a datetime si es string
        if isinstance(current_timestamp, str):
            try:
                current_timestamp = pl.select(pl.lit(current_timestamp).str.to_datetime(format='ISO8601')).item()
            except:
                return 'Desconocido'

        # Buscar el evento anterior en el mismo partido
        if current_idx > 0:
            current_match = self.df.filter(pl.col('row_idx') == current_idx)['Match ID'].item()

            # Buscar hacia atrás hasta encontrar un evento del mismo partido
            for prev_idx in range(int(current_idx) - 1, max(0, int(current_idx) - 20), -1):
                prev_row = self.df.filter(pl.col('row_idx') == prev_idx)
                if len(prev_row) > 0:
                    prev_match = prev_row['Match ID'].item()
                    prev_timestamp = prev_row['timeStamp'].item()

                    if prev_match == current_match and prev_timestamp is not None:
    ...
    def draw_pitch_with_throw_ins(self, ax, throw_ins_list, zone_number, side):
        ...
        for throw_in in throw_ins_list:
            ...
            # Validar coordenadas
            if x_start is None or (isinstance(x_start, float) and np.isnan(x_start)):
                continue
...
def seleccionar_equipo_interactivo():
    ...
    try:
        df = pl.read_parquet("extraccion_opta/datos_opta_parquet/open_play_events.parquet")
        equipos = sorted(df['Team Name'].drop_nulls().unique().to_list())
