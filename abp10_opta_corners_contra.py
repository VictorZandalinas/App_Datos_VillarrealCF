...
class CornersDefensivosReport:
    def __init__(self, data_path="extraccion_opta/datos_opta_parquet/abp_events.parquet"):
        ...
        self.corner_sequences = None  # Para secuencias defensivas
        
        self.load_data()
        if self.df_events is not None:
            self.extract_all_corner_sequences_defensivo()
    ...
    def extract_all_corner_sequences_defensivo(self):
        """Extrae córners CONTRA todos los equipos"""
        ...
        for match_id, match_events in df_sorted.groupby('Match ID'):
            ...
            # Córners de TODOS los equipos en este partido
            ...
            for _, lanzamiento in all_corners.iterrows():
                # Identificar el equipo que DEFIENDE el córner (el que NO lanza)
                equipo_lanzador = lanzamiento['Team Name']
                equipo_defensor = [eq for eq in equipos_partido if eq != equipo_lanzador][0]
                
                # Analizar la secuencia (misma función que abp3)
    ...
    def prepare_corner_metrics(self):
        """Prepara las métricas específicas de córners defensivos"""
        if self.df is None:
            return
        ...
        metrics_to_find = [
            'lostCorners',        # Córners en contra
            'goalsConceded',      # Goles concedidos
            'goalsConcededIbox',  # Goles concedidos en el área
            'goalsConcededObox',  # Goles concedidos fuera del área
            'Week',
            'saves',              # Paradas del portero
            'blockedScoringAtt'   # Remates bloqueados
        ]
    ...
    def get_team_corner_stats(self, equipo_seleccionado=None):
        ...
        stats_agrupadas = df_filtrado.groupby('Team Name').agg(
            num_corners_contra=('Team Name', 'size'),           
            goles_concedidos_corner=('result_type', lambda x: (x == 'Gol').sum()) 
        ).reset_index()
        ...
        stats_agrupadas['corners_contra_por_jornada'] = stats_agrupadas['num_corners_contra'] / stats_agrupadas['num_jornadas'].replace(0, 1)
        stats_agrupadas['goles_concedidos_por_corner'] = stats_agrupadas['goles_concedidos_corner'] / stats_agrupadas['num_corners_contra'].replace(0, 1)
        stats_agrupadas['lostCorners'] = stats_agrupadas['num_corners_contra']  # Alias para compatibilidad
    ...
    def calculate_derived_metrics(self, team_stats):
        """Calcula métricas derivadas para el reporte defensivo"""
        if team_stats is None:
            return None
        
        # Calcular métricas derivadas defensivas
        derived_stats = team_stats.copy()
        
        # Goles concedidos por córner
        if 'goalsConceded' in derived_stats.columns and 'lostCorners' in derived_stats.columns:
            derived_stats['goles_concedidos_por_corner'] = derived_stats['goalsConceded'] / derived_stats['lostCorners'].replace(0, 1)
    ...
    def create_visualization(self, equipo_seleccionado, figsize=(11.69, 8.27)):
        ...
        ax_title.text(0.5, 0.6, 'CÓRNERS DEFENSIVOS', 
        ...
        # Gráfico 1: Córners en contra por jornada (izquierda)
        ax_corners = fig.add_subplot(gs[1, 0])
        ax_corners.set_facecolor('none')
        ax_corners.set_title('Goles de córner\nen contra', fontsize=12, weight='bold', 
                  color='#1e3d59', pad=15)
        self.plot_goles_corner_contra(ax_corners, team_stats, equipo_seleccionado)
        
        # Gráfico 2: Goles concedidos por córner (centro)
        ax_goles_concedidos = fig.add_subplot(gs[1, 1])
        ax_goles_concedidos.set_facecolor('none')
        ax_goles_concedidos.set_title('Goles concedidos\npor córner', fontsize=12, weight='bold', 
                               color='#1e3d59', pad=15)
        self.plot_goles_concedidos_corner(ax_goles_concedidos, team_stats, equipo_seleccionado)  

        
        # Gráfico 3: Scatter plot (derecha)
        ax_scatter = fig.add_subplot(gs[1, 2])
        ax_scatter.set_facecolor('none')
        ax_scatter.set_title('Suma de Córners\nen contra', fontsize=12, weight='bold', 
                            color='#1e3d59', pad=15)
        self.plot_corner_scatter_defensivo(ax_scatter, team_stats, equipo_seleccionado)
        
        return fig
    
    def plot_goles_corner_contra(self, ax, team_stats, equipo_seleccionado):
        """Dibuja el gráfico de goles de córner en contra"""
        # Ordenar por goles de córner en contra
        sorted_data = team_stats.sort_values('goles_concedidos_corner', ascending=False)
        ...
        # Crear gráfico horizontal usando corners_contra_por_jornada
        bars = ax.barh(range(len(sorted_data)), sorted_data['goles_concedidos_corner'],
        ...
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['goles_concedidos_corner'], sorted_data['EQUIPO'])):
        ...
        ax.set_yticklabels(sorted_data['EQUIPO'], fontsize=8)  
        ax.set_xlabel('Goles de Córner en contra', fontsize=10, weight='bold')
        
        # Línea promedio
        promedio = sorted_data['goles_concedidos_corner'].mean()
    ...
    def plot_goles_concedidos_corner(self, ax, team_stats, equipo_seleccionado):
        """Dibuja el gráfico de goles concedidos por córner"""
        # INVERTIDO: menos goles concedidos por córner es mejor
        sorted_data = team_stats.sort_values('goles_concedidos_por_corner', ascending=False)
        ...
        bars = ax.barh(range(len(sorted_data)), sorted_data['goles_concedidos_por_corner'], 
                    color=colors, alpha=0.8, edgecolor='#2c3e50', linewidth=0.5)
        
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['goles_concedidos_por_corner'], sorted_data['EQUIPO'])):
            if team == equipo_seleccionado or 'villarreal' in team.lower():
                ax.text((valor) + 0.01, i, f'{float(valor):.2f}', va='center', fontsize=9, weight='bold')
        
        ax.set_yticks(range(len(sorted_data)))
        ax.set_yticklabels(sorted_data['EQUIPO'], fontsize=8)
        ax.set_xlabel('Goles concedidos por Córner', fontsize=10, weight='bold')
        
        promedio = sorted_data['goles_concedidos_por_corner'].mean()
        ax.axvline(promedio, color='#27ae60', linestyle='--', linewidth=2, alpha=0.7)
    ...
    def plot_corner_scatter_defensivo(self, ax, team_stats, equipo_seleccionado):
        """Dibuja el scatter plot: Córners en contra vs Goles concedidos por Córner"""
        x_data = team_stats['lostCorners']  # Suma de corners en contra
        y_data = team_stats['goles_concedidos_por_corner']  # Goles concedidos por córner
        ...
        for i, (equipo, x_val, y_val) in enumerate(zip(equipos, x_data, y_data)):
            ...
            if escudo is not None:
                try:
                    ...
                    imagebox = OffsetImage(escudo_bn, zoom=0.24, alpha=0.8)
        ...
        ax.set_xlabel('Suma de Córners en Contra', fontsize=12, weight='bold')
        ax.set_ylabel('Goles concedidos por Córner', fontsize=12, weight='bold')
...
def main():
    ...
    if fig:
        ...
        # Guardar como PDF
        output_path = "reporte_corners_defensivos.pdf"
