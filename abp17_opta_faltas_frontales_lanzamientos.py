...
try:
    ...
    print("âš ï¸ scipy no disponible - los mapas de calor no funcionarÃ¡n")
...
class FreeKicksAnalysis:
    def __init__(self, data_path="extraccion_opta/datos_opta_parquet/abp_events.parquet", team_filter=None):
        ...
        if team_filter:
            if self.df is not None:  # â† AÃ‘ADIR ESTA VERIFICACIÃ“N
                ...
                print("âŒ Error: No se pudieron cargar los datos")
    ...
    def load_player_photos(self):
        ...
        try:
            ...
            print("âš ï¸ No se encontrÃ³ el archivo jugadores_optimizados.json")
            return []
    ...
    def get_player_photo_with_team_filter(self, player_name, photos_data, team_filter):
        ...
        try:
            ...
            print(f"âš ï¸ Error procesando foto de {player_name}: {e}")
            return None
    ...
    def get_faltas_ranking_data(self, team_filter=None):
        ...
        team_data = self.free_kicks_data[
            ...
            (self.free_kicks_data['zone'].isin([8, 9, 10]))  # â† AÃ‘ADIR ESTA LÃNEA
    ...
    def create_faltas_ranking(self, ax, team_filter):
        ...
        # TÃ­tulo
        ax.text(0.5, 1.00, 'MEJORES LANZADORES\n FALTAS FRONTALES', fontsize=10, fontweight='bold',
    ...
    def load_data(self, team_filter=None):
        ...
        try:
            ...
            print(f"âœ… Datos filtrados cargados: {len(self.df)} eventos")
            ...
            if 'Free kick taken' in self.df.columns:
                ...
                print(f"Valores Ãºnicos en 'Free kick taken': {fk_values}")
            
            if 'Zone' in self.df.columns:
                print(f"Columna 'Zone' encontrada")
                zone_values = self.df['Zone'].value_counts()
                print(f"Valores Ãºnicos en 'Zone': {zone_values}")
    ...
    def extract_free_kicks_data(self, team_filter=None):
        """Extrae datos de faltas ofensivas frontales desde zonas 8, 9 y 10"""
        if self.df is None:
            print("â Œ No hay datos cargados")
            return

        print("ðŸ”  Extrayendo datos de faltas ofensivas desde zonas 8, 9 y 10...")
        ...
        # Esto es mÃ¡s eficiente que hacerlo dentro de un bucle.
        ...
        # Quitamos (df_sorted.get('Zone', '') == 'Center')
        # AÃ±adimos (df_sorted['start_zone'].isin([8, 9, 10]))
        free_kicks_directos = df_sorted[
            (df_sorted.get('Free kick taken', '') == 'Sí') &
            (df_sorted['x'] > 60) &
            (df_sorted['start_zone'].isin([8, 9, 10])) &  # <-- ESTE ES EL NUEVO FILTRO
            (df_sorted['x'].notna()) &
            (df_sorted['y'].notna())
        ]
        ...
        for _, free_kick in free_kicks_directos_unique.iterrows():
            try:
                ...
                # Ya no necesitamos volver a calcular la zona, la tomamos de la columna.
        ...
        if free_kicks_list:
            ...
            print(f"âœ… Total de faltas extraÃ­das desde zonas 8, 9 y 10: {len(self.free_kicks_data)}")
            ...
            print("â Œ No se encontraron faltas desde las zonas 8, 9 o 10.")
    ...
    def load_team_logo(self, equipo, target_size=(80, 80)):
        ...
        try:
            ...
            print("âš ï¸ PIL no estÃ¡ instalado. Usando mÃ©todo original sin redimensiÃ³n.")
        ...
        if logo_path:
            try:
                ...
                print(f"âš ï¸ Error procesando {logo_path}: {e}")
                return self._load_team_logo_original(equipo)
    ...
    def prepare_free_kicks_data(self, team_filter=None):
        ...
        if self.free_kicks_data.empty:
            print("âŒ No hay datos de faltas para preparar")
            return {}
        ...
        print("\nðŸ“Š Resumen de faltas preparadas por zona:")
    ...
    def get_caidas_por_zona(self, zona_lanzamiento):
        ...
        # Contar caÃ­das en cada zona usando Pass End X/Y
        for team_pos in ['home', 'away']:
            for point in data[team_pos]:
                end_x = point.get('result_x', 0)
                end_y = point.get('result_y', 0)
    ...
    def create_free_kicks_visualization(self, figsize=(11.69, 8.27), team_filter=None):
        ...
        fig.suptitle('ANÁLISIS DE FALTAS OFENSIVAS FRONTALES', 
        ...
        ax_zona9 = fig.add_subplot(gs[0, 1:3])  # Columnas centrales (1 y 2)
        ax_ranking = fig.add_subplot(gs[0, 3])   # Ãšltima columna

        # FILA INFERIOR: Zona 8 (columnas 1-2) y Zona 10 (columnas 3-4)
        ax_zona8 = fig.add_subplot(gs[1, 0:2])   # Primeras 2 columnas
        ax_zona10 = fig.add_subplot(gs[1, 2:4])  # Ãšltimas 2 columnas

        # ConfiguraciÃ³n de campos
        campos_config = [
            (ax_zona9, 9, 'CENTRO'),
            (ax_zona8, 8, 'CENTRO IZQUIERDA'), 
            (ax_zona10, 10, 'CENTRO DERECHA')
        ]
        ...
        legend_elements = [
            ...
            Line2D([0], [0], marker='s', color='w', markerfacecolor='red', markersize=8, 
                markeredgecolor='black', alpha=0.7, label='Zona caÃ­da (intensidad)'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='white', markersize=6,
                markeredgecolor='black', label='BalÃ³n (posiciÃ³n lanzamiento)'),
        ]
    ...
    def print_summary(self, team_filter=None):
        ...
        print(f"\n=== RESUMEN DE FALTAS OFENSIVAS FRONTALES ===")
        ...
        while True:
            try:
                ...
                if 0 <= indice < len(equipos):
                    ...
                    print(f"Por favor, ingresa un nÃºmero entre 1 y {len(equipos)}")
            except ValueError:
                print("Por favor, ingresa un nÃºmero vÃ¡lido")
    except Exception as e:
        print(f"Error en la selecciÃ³n: {e}")
        return None

def main():
    """FunciÃ³n principal"""
    try:
        print("=== GENERADOR DE REPORTES DE ANÃLISIS DE FALTAS OFENSIVAS FRONTALES ===")
        if (equipo := seleccionar_equipo_interactivo()) is None:
            print("No se pudo completar la selecciÃ³n.")
            return
        ...
        if (fig := analyzer.create_free_kicks_visualization(team_filter=equipo)):
            ...
            output_path = f"reporte_faltas_ofensivas_{equipo_filename}.pdf"
            analyzer.guardar_sin_espacios(fig, output_path)
            print(f"âœ… Reporte guardado como: {output_path}")
        else:
            print("âŒ No se pudo generar la visualizaciÃ³n")
...
def verificar_assets():
    ...
    for file_path in files_to_check:
        if os.path.exists(file_path):
            ...
            print(f"âŒ Faltante: {file_path}")
    
    if os.path.exists('assets/escudos'):
        escudos = [f for f in os.listdir('assets/escudos') if f.endswith('.png')]
        if escudos:
            print(f"âœ… Escudos disponibles ({len(escudos)}): {escudos[:5]}...")
        else:
            print("âš ï¸  No hay escudos en el directorio")
    else:
        print("âš ï¸  Directorio de escudos no existe")

if __name__ == "__main__":
    print("=== INICIALIZANDO GENERADOR DE REPORTES DE FALTAS OFENSIVAS FRONTALES ===")
    try:
        verificar_assets()
        df = pd.read_parquet("extraccion_opta/datos_opta_parquet/abp_events.parquet")
        equipos = sorted(df['Team Name'].dropna().unique())
        print(f"\nâœ… Sistema listo. Equipos disponibles: {len(equipos)}")
        if equipos:
            print("ðŸ” Para generar un reporte ejecuta: main()")
            print("ðŸ” Para uso directo: generar_reporte_personalizado('Nombre_Equipo')")
    except Exception as e:
