...
class FaltasDefensivasReport:
    ...
    def extract_all_freekick_indirect_sequences_defensivo(self):
        ...
        if lanzamientos_list:
            ...
            print(f"✅ Secuencias defensivas de faltas indirectas analizadas: {len(self.freekick_sequences)}")
    ...
    def get_team_freekick_indirect_stats(self, equipo_seleccionado=None):
        """Estadísticas DEFENSIVAS de faltas indirectas con xG"""
        ...
        # Calcular xG por equipo defensor (necesitarás tener xG en tus datos)
        xg_stats = []
        for equipo in equipos_liga:
            equipo_data = df_filtrado[df_filtrado['Team Name'] == equipo]
            
            total_xg = 0
            for _, row in equipo_data.iterrows():
                # Aquí necesitarás obtener el xG del evento de remate
                # Asumiendo que tienes acceso a datos de xG
                if row['result_type'] in ['Gol', 'Tiro a puerta', 'Tiro al poste', 'Tiro fuera']:
                    # Buscar xG correspondiente en tu dataset de xG
                    xg_value = self.get_xg_for_event(row)
                    total_xg += xg_value
            
            xg_stats.append({
                'Team Name': equipo,
                'xg_concedido_falta_indirecta': total_xg
            })
        
        xg_df = pd.DataFrame(xg_stats)
        ...
        # Fusionar con xG
        stats_agrupadas = stats_agrupadas.merge(xg_df, on='Team Name', how='left')
        stats_agrupadas['xg_concedido_falta_indirecta'] = stats_agrupadas['xg_concedido_falta_indirecta'].fillna(0)
        
        # Calcular número de jornadas por equipo
        ...
        stats_agrupadas['xg_concedido_por_falta_indirecta'] = stats_agrupadas['xg_concedido_falta_indirecta'] / stats_agrupadas['num_faltas_indirectas_contra'].replace(0, 1)
        ...
        stats_agrupadas['lostFreeKicksIndirect'] = stats_agrupadas['xg_concedido_falta_indirecta']  # Para scatter X
    ...
    def get_xg_for_event(self, event_row):
        """Obtiene el xG para un evento específico"""
        # Esta función necesitará ser adaptada según cómo tengas los datos de xG
        # Ejemplo básico:
        try:
            # Si tienes acceso al dataset de xG events como en abp1
            if hasattr(self, 'xg_events_df') and self.xg_events_df is not None:
                matching_xg = self.xg_events_df[
                    (self.xg_events_df['Match ID'] == event_row['Match ID']) &
                    (self.xg_events_df['Team ID'] == event_row['Team ID']) &
                    (self.xg_events_df['timeStamp'] == event_row.get('timeStamp', ''))
                ]
                
                if not matching_xg.empty:
                    return float(matching_xg.iloc[0]['qualifier 321'])
            
            # Si no tienes datos de xG, usar valores por defecto según tipo de remate
            result_type = event_row['result_type']
            if result_type == 'Gol':
                return 0.8  # Alta probabilidad para goles
            elif result_type == 'Tiro a puerta':
                return 0.3
            elif result_type == 'Tiro al poste':
                return 0.4
            elif result_type == 'Tiro fuera':
                return 0.1
            else:
                return 0.0
        except:
    ...
    def create_visualization(self, equipo_seleccionado, figsize=(11.69, 8.27)):
        """Crea la visualización completa del informe de xG faltas indirectas defensivas"""
        ...
        if team_stats is None:
            print("No se pudieron obtener las estadísticas de faltas indirectas defensivas")
            return None
        ...
        # Gráfico 1: xG en contra de falta indirecta (izquierda)
        ax_faltas = fig.add_subplot(gs[1, 0])
        ax_faltas.set_facecolor('none')
        ax_faltas.set_title('xG en contra\nde falta indirecta', fontsize=12, weight='bold', 
                        color='#1e3d59', pad=15)
        self.plot_xg_contra_falta_indirecta(ax_faltas, team_stats, equipo_seleccionado)
        
        # Gráfico 2: xG concedido por falta indirecta (centro)
        ax_goles_concedidos = fig.add_subplot(gs[1, 1])
        ax_goles_concedidos.set_facecolor('none')
        ax_goles_concedidos.set_title('xG en contra\npor falta indirecta', fontsize=12, weight='bold', 
                            color='#1e3d59', pad=15)
        self.plot_xg_concedido_por_falta_indirecta(ax_goles_concedidos, team_stats, equipo_seleccionado)  

        
        # Gráfico 3: Scatter plot (derecha)
        ax_scatter = fig.add_subplot(gs[1, 2])
        ax_scatter.set_facecolor('none')
        ax_scatter.set_title('Suma de xG en contra\nde falta indirecta', fontsize=12, weight='bold', 
                            color='#1e3d59', pad=15)
        self.plot_falta_indirecta_scatter_defensivo(ax_scatter, team_stats, equipo_seleccionado)
        
        return fig
    
    def plot_xg_contra_falta_indirecta(self, ax, team_stats, equipo_seleccionado):
        """Dibuja el gráfico de xG en contra de falta indirecta"""
        # Ordenar por xg_concedido_falta_indirecta (INVERTIDO: menos es mejor)
        sorted_data = team_stats.sort_values('xg_concedido_falta_indirecta', ascending=False)
        ...
        # Crear gráfico horizontal usando xg_concedido_falta_indirecta
        bars = ax.barh(range(len(sorted_data)), sorted_data['xg_concedido_falta_indirecta'], 
        ...
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['xg_concedido_falta_indirecta'], sorted_data['EQUIPO'])):
            if team == equipo_seleccionado or 'villarreal' in team.lower():
                ax.text((valor) + 0.02, i, f'{float(valor):.2f}', va='center', fontsize=9, weight='bold')
        
        # Configurar ejes
        ax.set_yticks(range(len(sorted_data)))
        ax.set_yticklabels(sorted_data['EQUIPO'], fontsize=8)  
        ax.set_xlabel('xG en contra de falta indirecta', fontsize=10, weight='bold')
        
        # Línea promedio
        promedio = sorted_data['xg_concedido_falta_indirecta'].mean()
    ...
    def plot_xg_concedido_por_falta_indirecta(self, ax, team_stats, equipo_seleccionado):
        """Dibuja el gráfico de xG concedido por falta indirecta"""
        # INVERTIDO: menos xG concedido por falta indirecta es mejor
        sorted_data = team_stats.sort_values('xg_concedido_por_falta_indirecta', ascending=False)
        ...
        bars = ax.barh(range(len(sorted_data)), sorted_data['xg_concedido_por_falta_indirecta'], 
                    color=colors, alpha=0.8, edgecolor='#2c3e50', linewidth=0.5)
        
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['xg_concedido_por_falta_indirecta'], sorted_data['EQUIPO'])):
            if team == equipo_seleccionado or 'villarreal' in team.lower():
                ax.text((valor) + 0.002, i, f'{float(valor):.3f}', va='center', fontsize=9, weight='bold')
        
        ax.set_yticks(range(len(sorted_data)))
        ax.set_yticklabels(sorted_data['EQUIPO'], fontsize=8)
        ax.set_xlabel('xG en contra por falta indirecta', fontsize=10, weight='bold')
        
        promedio = sorted_data['xg_concedido_por_falta_indirecta'].mean()
        ax.axvline(promedio, color='#27ae60', linestyle='--', linewidth=2, alpha=0.7)
    ...
    def plot_falta_indirecta_scatter_defensivo(self, ax, team_stats, equipo_seleccionado):
        """Dibuja el scatter plot: xG en contra vs Tiros por falta indirecta"""
        x_data = team_stats['lostFreeKicksIndirect']  # Suma de xG en contra de falta indirecta
        y_data = team_stats['tiros_por_falta_indirecta']  # Suma de tiros en contra / falta indirecta
        ...
        for i, (equipo, x_val, y_val) in enumerate(zip(equipos, x_data, y_data)):
            ...
            ax.text(x_val, y_val, equipo, ha='center', va='center', 
                    fontsize=fontsize, weight=weight, color=color, zorder=zorder_level,
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                            edgecolor=color, alpha=0.8))
        
        ax.set_xlabel('Suma de xG en contra de falta indirecta', fontsize=12, weight='bold')
        ax.set_ylabel('Suma de tiros en contra / falta indirecta', fontsize=12, weight='bold')
...
def main():
    ...
    print("=== GENERADOR DE REPORTES DE FALTAS DEFENSIVAS ===")
