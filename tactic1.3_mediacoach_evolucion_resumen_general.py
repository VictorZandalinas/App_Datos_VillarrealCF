...
class EvolucionRendimientoMediaCoach:
    ...
    def cargar_datos(self):
        ...
        try:
            self.df_stats = pl.read_parquet(self.parquet_path)
            self.df_stats = self.df_stats.with_columns([
                pl.col('VALOR').cast(pl.Utf8).str.replace(',', '.').cast(pl.Float64).alias('VALOR_num')
            ])
            self.df_stats = self.df_stats.filter(pl.col('VALOR_num').is_not_null())
            self.df_stats = self.df_stats.with_columns([
                pl.col('NOMBRE MÉTRICA').str.strip_chars().alias('NOMBRE MÉTRICA')
            ])
            # Reemplazar columna VALOR con la numérica
            self.df_stats = self.df_stats.drop('VALOR').rename({'VALOR_num': 'VALOR'})
    ...
    def obtener_equipos_disponibles(self):
        ...
        if self.df_stats is not None:
            return sorted(self.df_stats['EQUIPO'].unique().to_list())
        return []

    def calcular_estadisticas_liga(self):
        """Calcula estadísticas de la liga para normalización."""
        estadisticas = {}
        datos_partido = self.df_stats.filter(pl.col('PERIODO') == 'Total Partido')
        ...
        for grupo, lista_metricas in todas_metricas.items():
            for metrica in lista_metricas:
                datos_metrica = datos_partido.filter(pl.col('NOMBRE MÉTRICA') == metrica)
                if len(datos_metrica) > 0:
                    valores_por_equipo = datos_metrica.group_by('EQUIPO').agg(
                        pl.col('VALOR').mean().alias('media')
                    )
                    estadisticas[metrica] = {
                        'media_liga': valores_por_equipo['media'].mean(),
                        'maximo': valores_por_equipo['media'].max(),
                        'minimo': valores_por_equipo['media'].min()
                    }
        return estadisticas
    ...
    def calcular_rendimiento_jornada(self, equipo, jornada, tipo='ofensivo'):
        ...
        datos = self.df_stats.filter(
            (pl.col('EQUIPO') == equipo) &
            (pl.col('jornada') == jornada) &
            (pl.col('PERIODO') == 'Total Partido')
        )
        ...
        for grupo, metricas in grupos.items():
            for metrica in metricas:
                valor_df = datos.filter(pl.col('NOMBRE MÉTRICA') == metrica)['VALOR']
                if len(valor_df) > 0:
                    valor = valor_df[0]
                    punt = self.calcular_puntuacion_metrica(valor, metrica, estadisticas)
    ...
    def obtener_rival_jornada(self, equipo, jornada):
        ...
        datos_equipo = self.df_stats.filter(
            (pl.col('EQUIPO') == equipo) &
            (pl.col('jornada') == jornada)
        )
        ...
        id_partido = datos_equipo['ID PARTIDO'][0]

        # Buscar el otro equipo en ese partido
        equipos_partido = self.df_stats.filter(
            pl.col('ID PARTIDO') == id_partido
        )['EQUIPO'].unique().to_list()
    ...
    def es_partido_local(self, equipo, jornada):
        ...
        if 'partido' not in self.df_stats.columns:
            # Si no existe la columna, usar método alternativo
            id_partido = datos_equipo['ID PARTIDO'][0]
            todos_equipos = self.df_stats.filter(
                pl.col('ID PARTIDO') == id_partido
            )['EQUIPO'].unique().to_list()
            return equipo == sorted(todos_equipos)[0]

        partido_str = datos_equipo['partido'][0].lower()
    ...
    def calcular_ranking_liga_metrica(self, metrica):
        ...
        promedios = datos.group_by('EQUIPO').agg(
        ...
        promedios_ordenados = promedios.sort(
            'promedio', descending=not es_invertida
        )

        # Crear ranking (1 = mejor)
        ranking = {}
        for idx, row in enumerate(promedios_ordenados.iter_rows(named=True)):
            ranking[row['EQUIPO']] = idx + 1
...
class VisualizadorEvolucion:
    ...
    def crear_informe_evolucion(self, equipo1, equipo2):
        ...
        # Filtrar valores vacíos y None antes de ordenar
        jornadas = [j for j in self.analyzer.df_stats['jornada'].unique().to_list() if j and str(j).strip()]
        jornadas = sorted(jornadas, key=extraer_numero_jornada)
        ...
        ax_fondo = fig.add_axes([0.055, 0.1, 0.89, 0.35], zorder=0)
    ...
    def crear_grafico_evolucion_dual(self, ax, equipo, jornadas, color_equipo):
        ...
        ax.set_xlabel('Jornada', fontsize=11, fontweight='bold')
        ax.set_xticks(x)
        # Extraer solo el número de la jornada (eliminar prefijo J/j)
        ax.set_xticklabels([str(j).replace("j", "").replace("J", "") for j in x], fontsize=9)
