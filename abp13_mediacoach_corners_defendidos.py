...
class CornersDefensivosReport:
    ...
    def clean_and_filter_data(self):
        ...
        # Filtrar por la métrica específica y período (EN CONTRA)
        self.df = self.df[
            (self.df['NOMBRE MÉTRICA'] == 'B.P Saque de esquina en contra (% Neutralizados)') &
    ...
    def get_all_teams_data(self, jornada_hasta):
        ...
        # Métricas necesarias
        df_neutralizados = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P Saque de esquina en contra (% Neutralizados)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        df_exito = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P Saque de esquina en contra (% Éxito)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        df_goles_bp = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'Goles en contra balón parado (% Total)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        df_exito_acciones = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P Acciones totales en contra (% Éxito)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()
        
        # Limpiar nombres de equipos
        for df in [df_neutralizados, df_exito, df_goles_bp, df_exito_acciones]:
        ...
        neutralizados_avg = df_neutralizados.groupby('EQUIPO')['VALOR'].mean().reset_index()
        neutralizados_avg.columns = ['EQUIPO', 'NEUTRALIZADOS']
        
        exito_avg = df_exito.groupby('EQUIPO')['VALOR'].mean().reset_index()
        exito_avg.columns = ['EQUIPO', 'EXITO']
        
        goles_bp_avg = df_goles_bp.groupby('EQUIPO')['VALOR'].mean().reset_index()
        goles_bp_avg.columns = ['EQUIPO', 'GOLES_BP']
        
        exito_acciones_avg = df_exito_acciones.groupby('EQUIPO')['VALOR'].mean().reset_index()
        exito_acciones_avg.columns = ['EQUIPO', 'EXITO_ACCIONES']
        
        # Combinar todas las métricas
        combined_data = pd.merge(neutralizados_avg, exito_avg, on='EQUIPO', how='inner')
        combined_data = pd.merge(combined_data, goles_bp_avg, on='EQUIPO', how='left')
        combined_data = pd.merge(combined_data, exito_acciones_avg, on='EQUIPO', how='left')
    ...
    def load_any_team_logo(self, equipo):
        """Intenta cargar el escudo de cualquier equipo con búsqueda mejorada y filtro de palabras largas"""
        ...
        def create_variants(team_name):
            """Crea todas las variantes posibles del nombre, filtrando palabras de más de 4 letras"""
            variants = []
            clean_name = clean_team_name(team_name)
            
            # Separar palabras y filtrar las de más de 4 letras
            words = clean_name.split()
            long_words = [word for word in words if len(word) > 4]
            
            # Si no hay palabras largas, usar el nombre completo
            if not long_words:
                long_words = words
            ...
            # Añadir solo palabras largas (más de 4 letras)
            ...
            for prefix in prefixes_to_remove:
                if clean_name.startswith(prefix):
                    ...
                    if len(core_name) > 4:  # Solo si el núcleo es largo
                        variants.extend([
                            core_name,
                            core_name.replace(' ', '_'),
                            core_name.replace(' ', ''),
            ...
            specific_mappings = {
                ...
                'rayo vallecano': ['vallecano', 'rayo_vallecano', 'rayovallecano'],
        ...
        print(f"Buscando escudo para '{equipo}'. Variantes (palabras >4 letras): {possible_names[:10]}...")
        ...
        # Si no encuentra nada, buscar por similitud solo con palabras largas
        if os.path.exists('assets/escudos'):
            escudos_disponibles = [f.replace('.png', '') for f in os.listdir('assets/escudos') if f.endswith('.png')]
            ...
            for variant in possible_names:
                # Solo procesar variantes de palabras largas
                if len(variant) <= 4:
                    continue
                ...
                for escudo_file in escudos_disponibles:
                    ...
                    if max_similarity > best_similarity and max_similarity > 0.6:  # Umbral 60%
            ...
            if best_match:
                ...
                try:
                    ...
                    print(f"Error al cargar escudo {best_match}: {e}")
                    return None
            else:
    ...
    def create_visualization(self, equipo, jornada_hasta, figsize=(11.69, 8.27)): 
        ...
        gs = fig.add_gridspec(3, 4, 
                    ...
                    hspace=0.35, wspace=0.4,  # Mismo espaciado que abp6
        ...
        # Título principal
        ax_title.text(0.5, 0.8, 'CÓRNERS DEFENDIDOS', 
                     fontsize=26, weight='bold', ha='center', va='center',
                     color='#1e3d59', family='serif')
        ax_title.text(0.5, 0.17, f'DESDE JORNADA 1 HASTA JORNADA {jornada_hasta}', 
        ...
        if team_logo is not None:
            try:
                imagebox = OffsetImage(team_logo, zoom=0.3)  # Cambiar de 0.5 a 0.3
                ab = AnnotationBbox(imagebox, (0.95, 0.7), frameon=False, pad=0)  # Cambiar posición Y
                ax_title.add_artist(ab)
            except Exception as e:
        ...
        if villarreal_logo is not None:
            try:
                imagebox = OffsetImage(villarreal_logo, zoom=0.3)  # Cambiar de 0.5 a 0.3
                ab = AnnotationBbox(imagebox, (0.92, 0.7), frameon=False, pad=0)  # Cambiar posición
                ax_title.add_artist(ab)
            except Exception as e:
        ...
        ax_evolution.set_title(f'PORCENTAJE DE CÓRNERS NEUTRALIZADOS POR PARTIDO - {equipo.upper()}', 
        ...
        ax_ranking.set_title('MEDIA DE CÓRNERS NEUTRALIZADOS', 
                            fontsize=12, weight='bold', color='#1e3d59', pad=10)
    ...
    def plot_evolution_chart(self, ax, filtered_df, jornada_hasta):
        """Dibuja el gráfico de evolución por jornadas con colores rojos para defensivos"""
        ...
        ax.set_ylabel('Nº Córners en contra', fontsize=10, weight='bold')
    ...
    def plot_general_stats(self, ax, filtered_df, all_teams_data, equipo):
        ...
        def get_team_metrics(team_name, jornada_hasta):
            ...
            # Métrica 1: % Neutralizados corners
            neutralizados_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P Saque de esquina en contra (% Neutralizados)']
            neutralizados = neutralizados_data['VALOR'].mean() if len(neutralizados_data) > 0 else 0

            # Métrica 2: % Goles BP (invertir porque menor es mejor para defensivo)
            goles_bp_data = team_data[team_data['NOMBRE MÉTRICA'] == 'Goles en contra balón parado (% Total)']
            goles_bp = goles_bp_data['VALOR'].mean() if len(goles_bp_data) > 0 else 0
            
            # Métrica 3: % Éxito acciones (invertir porque menor es mejor para defensivo)
            exito_acciones_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P Acciones totales en contra (% Éxito)']
            exito_acciones = exito_acciones_data['VALOR'].mean() if len(exito_acciones_data) > 0 else 0
            
            return {
                'neutralizados': neutralizados,
                'goles_bp': goles_bp,
                'exito_acciones': exito_acciones
            }
        ...
        # Calcular métricas para todos los equipos para obtener rangos
        all_teams_metrics = []
        teams_with_names = []  # Para mantener los nombres de equipos
        for team in equipos_unicos:
            team_metrics = get_team_metrics(team, jornada_hasta)
            if team_metrics:
                all_teams_metrics.append(team_metrics)
                teams_with_names.append((team, team_metrics))
        
        if len(all_teams_metrics) > 0:
            max_neutralizados = max([m['neutralizados'] for m in all_teams_metrics])
            min_neutralizados = min([m['neutralizados'] for m in all_teams_metrics])
            max_goles_bp = max([m['goles_bp'] for m in all_teams_metrics])
            min_goles_bp = min([m['goles_bp'] for m in all_teams_metrics])
            max_exito_acciones = max([m['exito_acciones'] for m in all_teams_metrics])
            min_exito_acciones = min([m['exito_acciones'] for m in all_teams_metrics])
        else:
            # Valores por defecto si no hay datos
            max_neutralizados, min_neutralizados = 100, 0
            max_goles_bp, min_goles_bp = 50, 0
            max_exito_acciones, min_exito_acciones = 50, 0
        
        # CALCULAR RANKINGS
        # Ordenar por neutralizados (descendente, mayor es mejor)
        neutralizados_ranking = sorted(teams_with_names, key=lambda x: x[1]['neutralizados'], reverse=True)
        
        # Encontrar ranking del equipo seleccionado
        ranking_equipo = len(neutralizados_ranking)  # Por defecto último
        ranking_villarreal = len(neutralizados_ranking)  # Por defecto último
        
        for rank, (team_name, metrics) in enumerate(neutralizados_ranking):
            if team_name == equipo:
        ...
        # Función para normalizar ranking (invertir para que 1º sea 100 y último sea 0)
        def normalize_ranking(rank, total_teams):
            if total_teams <= 1:
                return 50
            return 100 - ((rank - 1) / (total_teams - 1) * 100)
        
        # Valores del equipo seleccionado (normalizados) - AHORA CON 4 VALORES
        values_equipo = [
            normalize_value(metrics_equipo['neutralizados'], min_neutralizados, max_neutralizados),
            100 - normalize_value(metrics_equipo['goles_bp'], min_goles_bp, max_goles_bp),  # Invertir: menor es mejor
            100 - normalize_value(metrics_equipo['exito_acciones'], min_exito_acciones, max_exito_acciones),  # Invertir: menor es mejor
            normalize_ranking(ranking_equipo, len(neutralizados_ranking))  # CUARTA MÉTRICA: RANKING
        ]

        # Valores de Villarreal (normalizados) - AHORA CON 4 VALORES
        values_villarreal = [
            normalize_value(metrics_villarreal['neutralizados'], min_neutralizados, max_neutralizados),
            100 - normalize_value(metrics_villarreal['goles_bp'], min_goles_bp, max_goles_bp),
            100 - normalize_value(metrics_villarreal['exito_acciones'], min_exito_acciones, max_exito_acciones),
            normalize_ranking(ranking_villarreal, len(neutralizados_ranking))  # CUARTA MÉTRICA: RANKING
        ]

        # Valores reales para mostrar en los rectángulos - AHORA CON 4 VALORES
        valores_reales_equipo = [
            metrics_equipo['neutralizados'],
            metrics_equipo['goles_bp'],
            metrics_equipo['exito_acciones'],
            ranking_equipo  # RANKING REAL
        ]

        valores_reales_villarreal = [
            metrics_villarreal['neutralizados'],
            metrics_villarreal['goles_bp'],
            metrics_villarreal['exito_acciones'],
            ranking_villarreal  # RANKING REAL
        ]
        ...
        # Quitar las etiquetas automáticas
        ax.set_xticks([])
        ax.set_xticklabels([])

        # Colocar etiquetas manualmente más cerca del centro
        metrics_labels = [
            '% Neutralizados\nCorners', 
            '% Goles BP\n(En contra)', 
            '% Éxito Acciones\n(En contra)',
        ...
        # Colocar etiquetas en un radio menor (más cerca del centro)
        label_radius = 115  # Cambiar este valor para ajustar la distancia (menor = más cerca)

        for angle, label in zip(angles[:-1], metrics_labels):
            ...
            ax.text(angle, label_radius, label, 
                ...
                rotation=0)  # Puedes ajustar la rotación si quieres
        ...
        # Equipo seleccionado (rojo para defensivo)
        ...
        # Añadir valores organizados: Villarreal (izq) - Equipo (der)
        for i, (val_villarreal, val_equipo) in enumerate(zip(valores_reales_villarreal, valores_reales_equipo)):
            angle = angles[i]
            radius_base = 110  # Mismo radio para todos
            
            # Formatear valores según la métrica
            if i == 3:  # Ranking (sin %)
                texto_villarreal = f"{val_villarreal:.0f}º"
                texto_equipo = f"{val_equipo:.0f}º"
            else:  # Porcentajes
            ...
            # Villarreal a la izquierda, equipo a la derecha
    ...
    def plot_team_ranking(self, ax, all_teams_data, selected_equipo):
        """Dibuja el ranking de equipos por % Neutralizados"""
        if all_teams_data is None or len(all_teams_data) == 0:
            ax.text(0.5, 0.5, 'No hay datos disponibles', ha='center', va='center')
            return
        
        # Ordenar equipos por % neutralizados (descendente, mayor es mejor)
        sorted_data = all_teams_data.sort_values('NEUTRALIZADOS', ascending=True)
        ...
        bars = ax.barh(range(len(sorted_data)), sorted_data['NEUTRALIZADOS'], 
                    color=colors, alpha=0.8, edgecolor='#2c3e50', linewidth=0.5)
        
        # Añadir valores solo para Villarreal y equipo seleccionado
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['NEUTRALIZADOS'], sorted_data['EQUIPO'])):
            if team == selected_equipo or 'villarreal' in team.lower():
                ax.text(valor + 0.5, i, f'{valor:.1f}%', va='center', fontsize=9, weight='bold')
        
        # Configurar ejes
        ax.set_yticks(range(len(sorted_data)))
        ax.set_yticklabels(sorted_data['EQUIPO'], fontsize=8)
        ax.set_xlabel('% Neutralizados', fontsize=10, weight='bold')
        
        # Línea promedio de la liga
        promedio_liga = sorted_data['NEUTRALIZADOS'].mean()
        ax.axvline(promedio_liga, color='#27ae60', linestyle='--', linewidth=2, alpha=0.7)
        ax.text(promedio_liga, len(sorted_data) * 0.95, f'Media Liga: {promedio_liga:.1f}%', 
    ...
    def plot_team_scatter(self, ax, all_teams_data, selected_equipo):
        """Dibuja el scatter plot: X=% Éxito, Y=% Neutralizados"""
        if all_teams_data is None or len(all_teams_data) == 0:
            ax.text(0.5, 0.5, 'No hay datos disponibles', ha='center', va='center')
            return
        
        # Extraer datos
        x_data = all_teams_data['EXITO']          # % Éxito en X
        y_data = all_teams_data['NEUTRALIZADOS']  # % Neutralizados en Y
        ...
        ax.text(x_promedio + 0.5, max(y_data) * 0.95, f'Media Éxito: {x_promedio:.1f}%', 
            rotation=90, ha='left', va='top', fontsize=8, color='#34495e', weight='bold')
        ax.text(max(x_data) * 0.98, y_promedio + 1, f'Media Neutralizados: {y_promedio:.1f}%', 
            ha='right', va='bottom', fontsize=8, color='#34495e', weight='bold')
        
        # Crear scatter plot con escudos y nombres
        for i, (equipo_name, x_val, y_val) in enumerate(zip(equipos, x_data, y_data)):
            # Definir si es equipo destacado
            is_selected = equipo_name == selected_equipo
            is_villarreal = 'villarreal' in equipo_name.lower()
            is_highlighted = is_selected or is_villarreal
            
            # SALTAR equipos destacados en este bucle
            if is_highlighted:
                continue
            
            # Cargar escudo (equipos normales)
            escudo = self.load_any_team_logo(equipo_name)
            
            # Configurar tamaño y procesamiento de imagen (equipos normales)
            zoom_size = 0.12
            alpha_val = 0.8
            # Convertir a escala de grises
            ...
            if processed_escudo is not None:
                try:
                    ...
                    print(f"Error al cargar escudo para {equipo_name}: {e}")
                    processed_escudo = None
            
            if processed_escudo is None:
                # Si no hay escudo, mostrar nombre del equipo (equipos normales)
                fontsize = 7
                weight = 'normal'
                color = '#2c3e50'
                
                ax.text(x_val, y_val, equipo_name, ha='center', va='center', fontsize=fontsize, 
                    weight=weight, color=color, 
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                            edgecolor=color, alpha=0.8))

        # SEGUNDO BUCLE: Dibujar equipos destacados ENCIMA
        for i, (equipo_name, x_val, y_val) in enumerate(zip(equipos, x_data, y_data)):
            # Definir si es equipo destacado
            is_selected = equipo_name == selected_equipo
            is_villarreal = 'villarreal' in equipo_name.lower()
            ...
            # SOLO procesar equipos destacados en este bucle
            ...
            # Configurar tamaño y procesamiento de imagen (equipos destacados)
            zoom_size = 0.24
            alpha_val = 1.0
            ...
            if processed_escudo is not None:
                try:
                    ...
                    ab = AnnotationBbox(imagebox, (x_val, y_val), 
                                    ...
                                    pad=0,
                                    boxcoords="data",
                                    zorder=10)  # Z-order alto para estar encima
                    ax.add_artist(ab)
                except Exception as e:
                    print(f"Error al cargar escudo para {equipo_name}: {e}")
                    processed_escudo = None
            ...
            if processed_escudo is None:
                # Si no hay escudo, mostrar nombre del equipo (equipos destacados)
                fontsize = 9
                weight = 'bold'
        ...
        # Configurar ejes
        ax.set_xlabel('% Éxito Corners', fontsize=10, weight='bold')
        ax.set_ylabel('% Neutralizados', fontsize=10, weight='bold')
        
        # Etiquetas de cuadrantes
        ax.text(0.02, 0.98, 'Altos Neutral.\nBajo Éxito', transform=ax.transAxes, 
            fontsize=8, ha='left', va='top', style='italic', color='#27ae60')
        ax.text(0.98, 0.98, 'Altos Neutral.\nAlto Éxito\n(Ideal)', transform=ax.transAxes, 
            fontsize=8, ha='right', va='top', style='italic', color='#2ecc71')
        ax.text(0.02, 0.02, 'Bajos Neutral.\nBajo Éxito\n(Problemático)', transform=ax.transAxes, 
            fontsize=8, ha='left', va='bottom', style='italic', color='#e74c3c')
        ax.text(0.98, 0.02, 'Bajos Neutral.\nAlto Éxito', transform=ax.transAxes, 
...
def main():
    try:
        ...
        if fig:
            ...
            # Guardar como PDF usando la función A4
