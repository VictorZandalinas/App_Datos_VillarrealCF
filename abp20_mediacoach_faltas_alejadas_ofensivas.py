...
class FaltasAlejadasReport:
    ...
    def __init__(self, data_path="extraccion_mediacoach/data/estadisticas_equipo.parquet"):
        """
        Inicializa la clase para generar informes de faltas ofensivas frontales
        """
    ...
    def clean_and_filter_data(self):
        ...
        self.df = self.df[
            (self.df['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (Nº)') &  # Cambiar la métrica
    ...
    def get_all_teams_data(self, jornada_hasta):
        ...
        # Filtrar por las métricas que necesitamos para faltas frontales
        df_faltas_alejadas = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (Nº)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()

        df_exito_alejadas = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (% Éxito)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()

        df_rematados_alejadas = df_original[
            (df_original['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (% Rematados)') &
            (df_original['PERIODO'] == 'Total Partido') &
            (df_original['jornada'].isin(jornadas_incluir))
        ].copy()

        
        # Limpiar nombres de equipos en todos los dataframes
        for df in [df_faltas_alejadas, df_exito_alejadas, df_rematados_alejadas]:
        ...
        # Calcular totales y número de partidos únicos por equipo usando ID PARTIDO
        if 'ID PARTIDO' in df_faltas_alejadas.columns:
            faltas_total = df_faltas_alejadas.groupby('EQUIPO').agg({
                'VALOR': 'sum',
                'ID PARTIDO': 'nunique'  # Contar partidos únicos
            }).reset_index()
            faltas_total.columns = ['EQUIPO', 'FALTAS_TOTAL', 'PARTIDOS']
        else:
            # Fallback usando jornadas si no hay ID PARTIDO
            faltas_total = df_faltas_alejadas.groupby('EQUIPO').agg({
                'VALOR': 'sum',
                'jornada': 'count'
            }).reset_index()
        ...
        faltas_total['FALTAS_POR_PARTIDO'] = faltas_total['FALTAS_TOTAL'] / faltas_total['PARTIDOS']
        
        # Calcular promedios para las otras métricas
        exito_promedio = df_exito_alejadas.groupby('EQUIPO').agg({
            'VALOR': 'mean'
        }).reset_index()
        exito_promedio.columns = ['EQUIPO', 'EXITO_PROMEDIO']
        
        rematados_promedio = df_rematados_alejadas.groupby('EQUIPO').agg({
            'VALOR': 'mean'
        }).reset_index()
        rematados_promedio.columns = ['EQUIPO', 'REMATADOS_PROMEDIO']
        
        # Combinar todas las métricas
        combined_data = pd.merge(
            faltas_total[['EQUIPO', 'FALTAS_POR_PARTIDO']], 
            rematados_promedio[['EQUIPO', 'REMATADOS_PROMEDIO']], 
            on='EQUIPO', 
            how='inner'
        )
        
        # Agregar métricas adicionales para el radar
        combined_data = pd.merge(
            combined_data,
            exito_promedio[['EQUIPO', 'EXITO_PROMEDIO']],
            on='EQUIPO',
            how='inner'
        )
        
        combined_data.columns = ['EQUIPO', 'FALTAS_ALEJADAS', 'REMATADOS', 'EXITO']
    ...
    def create_visualization(self, equipo, jornada_hasta, figsize=(11.69, 8.27)):
        """Crea la visualización completa del informe de faltas ofensivas frontales"""
        ...
        # Título principal
        ax_title.text(0.5, 0.8, 'FALTAS OFENSIVAS ALEJADAS', 
        ...
        ax_evolution.set_title(f'FALTAS ALEJADAS POR PARTIDO - {equipo.upper()}', 
        ...
        ax_ranking.set_title('MEDIA DE FALTAS ALEJADAS A FAVOR', 
                            fontsize=12, weight='bold', color='#1e3d59', pad=10)
    ...
    def plot_evolution_chart(self, ax, filtered_df, jornada_hasta):
        ...
        ax.set_ylabel('Nº Faltas Alejadas', fontsize=10, weight='bold')
    ...
    def plot_general_stats(self, ax, filtered_df, all_teams_data, equipo):
        ...
        def get_team_metrics(team_name, jornada_hasta):
            ...
            # Contar número de partidos únicos usando ID PARTIDO si está disponible
            if 'ID PARTIDO' in team_data.columns:
                num_partidos = len(team_data['ID PARTIDO'].unique())
            else:
                # Fallback: usar jornadas diferentes
                num_partidos = len(team_data['jornada'].unique())
            
            if num_partidos == 0:
                return None
            
            # Métrica 1: Faltas frontales por partido
            faltas_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (Nº)']
            faltas_total = faltas_data['VALOR'].sum()
            faltas_por_partido = faltas_total / num_partidos if num_partidos > 0 else 0
            
            # Métrica 2: % Éxito de faltas frontales
            exito_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (% Éxito)']
            if len(exito_data) > 0:
                exito_promedio = exito_data['VALOR'].mean()
            else:
                exito_promedio = 0
            
            # Métrica 3: % Rematados de faltas frontales
            rematados_data = team_data[team_data['NOMBRE MÉTRICA'] == 'B.P F. Alejadas a favor – hacia zona ataque (% Rematados)']
            if len(rematados_data) > 0:
                rematados_promedio = rematados_data['VALOR'].mean()
            else:
                rematados_promedio = 0
            
            return {
                'faltas_por_partido': faltas_por_partido,
                'exito_promedio': exito_promedio,
                'rematados_promedio': rematados_promedio,
        ...
        if villarreal_name:
            ...
            metrics_villarreal = {
                'faltas_por_partido': 3.0,
                'exito_promedio': 15.0,
                'rematados_promedio': 40.0,
                'num_partidos': 5
            }
        ...
        # Calcular rankings para todas las métricas
        teams_for_ranking = []
        for team in equipos_unicos:
            team_metrics = get_team_metrics(team, jornada_hasta)
            if team_metrics:
                # Ranking combinado: promedio de las 3 métricas normalizadas
                combined_score = (
                    team_metrics['faltas_por_partido'] * 0.33 +
                    team_metrics['exito_promedio'] * 0.33 +
                    team_metrics['rematados_promedio'] * 0.33
                )
                teams_for_ranking.append((team, combined_score))
        
        # Ordenar por score combinado (descendente) para obtener rankings
        ...
        for rank, (team_name, score) in enumerate(teams_for_ranking):
            if team_name == equipo:
        ...
        if len(all_teams_metrics) > 0:
            max_faltas = max([m['faltas_por_partido'] for m in all_teams_metrics])
            min_faltas = min([m['faltas_por_partido'] for m in all_teams_metrics])
            max_exito = max([m['exito_promedio'] for m in all_teams_metrics])
            min_exito = min([m['exito_promedio'] for m in all_teams_metrics])
            max_rematados = max([m['rematados_promedio'] for m in all_teams_metrics])
            min_rematados = min([m['rematados_promedio'] for m in all_teams_metrics])
            max_ranking = len(teams_for_ranking)  # Número total de equipos
        else:
            # Valores por defecto si no hay datos
            max_faltas, min_faltas = 10, 0
            max_exito, min_exito = 50, 0
            max_rematados, min_rematados = 100, 0
            max_ranking = 20
        ...
        values_equipo = [
            normalize_value(metrics_equipo['faltas_por_partido'], min_faltas, max_faltas),
            normalize_value(metrics_equipo['exito_promedio'], min_exito, max_exito),
            normalize_value(metrics_equipo['rematados_promedio'], min_rematados, max_rematados),
        ...
        values_villarreal = [
            normalize_value(metrics_villarreal['faltas_por_partido'], min_faltas, max_faltas),
            normalize_value(metrics_villarreal['exito_promedio'], min_exito, max_exito),
            normalize_value(metrics_villarreal['rematados_promedio'], min_rematados, max_rematados),
            # Para el ranking, invertir la normalización (ranking 1 = 100, ranking máximo = 0)
        ...
        valores_reales_equipo = [
            metrics_equipo['faltas_por_partido'],
            metrics_equipo['exito_promedio'],
            metrics_equipo['rematados_promedio'],
            ranking_equipo
        ]

        valores_reales_villarreal = [
            metrics_villarreal['faltas_por_partido'],
            metrics_villarreal['exito_promedio'],
            metrics_villarreal['rematados_promedio'],
            ranking_villarreal
        ]
        ...
        for i, (val_villarreal, val_equipo) in enumerate(zip(valores_reales_villarreal, valores_reales_equipo)):
            ...
            if i == 1 or i == 2:  # % Éxito y % Rematados
    ...
    def plot_team_ranking(self, ax, all_teams_data, selected_equipo):
        ...
        # Ordenar equipos por promedio de faltas frontales
        sorted_data = all_teams_data.sort_values('FALTAS_ALEJADAS', ascending=True)
        ...
        bars = ax.barh(range(len(sorted_data)), sorted_data['FALTAS_ALEJADAS'], 
                    color=colors, alpha=0.8, edgecolor='#2c3e50', linewidth=0.5)
        
        # Añadir valores solo para Villarreal y equipo seleccionado
        for i, (bar, valor, team) in enumerate(zip(bars, sorted_data['FALTAS_ALEJADAS'], sorted_data['EQUIPO'])):
        ...
        ax.set_xlabel('Media faltas alejadas a favor', fontsize=10, weight='bold')
        
        # Línea promedio de la liga
        promedio_liga = sorted_data['FALTAS_ALEJADAS'].mean()
    ...
    def plot_team_scatter(self, ax, all_teams_data, selected_equipo):
        """Dibuja el scatter plot con faltas frontales vs % rematados, cruz divisoria y escudos"""
        if all_teams_data is None or len(all_teams_data) == 0:
            ax.text(0.5, 0.5, 'No hay datos disponibles', ha='center', va='center')
            return
        
        # Extraer datos
        x_data = all_teams_data['FALTAS_ALEJADAS']  # Faltas frontales por partido
        y_data = all_teams_data['REMATADOS']         # % Rematados
        ...
        ax.text(max(x_data) * 0.98, y_promedio + 1, f'Media % rematados: {y_promedio:.1f}%', 
        ...
        for i, (equipo, x_val, y_val) in enumerate(zip(equipos, x_data, y_data)):
            ...
            if processed_escudo is not None:
                try:
                    # Definir zorder antes del AnnotationBbox
        ...
        # Configurar ejes
        ax.set_xlabel('Media faltas alejadas a favor', fontsize=10, weight='bold')
        ax.set_ylabel('% Rematados promedio', fontsize=10, weight='bold')
        ...
        ax.text(0.02, 0.98, 'Alto % Rematados\nPocas Faltas', transform=ax.transAxes, 
            fontsize=8, ha='left', va='top', style='italic', color='#7f8c8d')
        ax.text(0.98, 0.98, 'Alto % Rematados\nMuchas Faltas', transform=ax.transAxes, 
            fontsize=8, ha='right', va='top', style='italic', color='#7f8c8d')
        ax.text(0.02, 0.02, 'Bajo % Rematados\nPocas Faltas', transform=ax.transAxes, 
            fontsize=8, ha='left', va='bottom', style='italic', color='#7f8c8d')
        ax.text(0.98, 0.02, 'Bajo % Rematados\nMuchas Faltas', transform=ax.transAxes, 
...
def seleccionar_equipo_interactivo():
    ...
    try:
        report_generator = FaltasAlejadasReport()
...
def main():
    try:
        print("=== GENERADOR DE REPORTES DE FALTAS OFENSIVAS ALEJADAS ===")
        ...
        if fig:
            ...
            output_path = f"reporte_faltas_alejadas_{equipo_filename}.pdf"
...
# Inicialización
print("=== INICIALIZANDO GENERADOR DE REPORTES FALTAS LATERALES ===")
try:
    verificar_assets()
    report_generator = FaltasAlejadasReport()
    equipos = report_generator.get_available_teams()
