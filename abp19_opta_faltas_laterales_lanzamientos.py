...
try:
    ...
    print("⚠️ scipy no disponible - los mapas de calor no funcionarán")
...
class FreeKicksAnalysis:
    def __init__(self, data_path="extraccion_opta/datos_opta_parquet/abp_events.parquet", team_filter=None):
        ...
        # Definir zonas de faltas según las coordenadas proporcionadas
        self.zones = {
        ...
        if team_filter:
            if self.df is not None:  # ← AÑADIR ESTA VERIFICACIÓN
    ...
    def match_player_name(self, player_name, photos_data, team_filter=None):
        ...
        def calculate_match_score(player_parts, photo_parts):
            """Calcula el score de coincidencia con reglas específicas."""
    ...
    def get_faltas_ranking_data(self, team_filter=None):
        ...
        team_data = self.free_kicks_data[
            ...
            (self.free_kicks_data['zone'].isin([1, 6, 7, 11]))  # ← ZONAS LATERALES
        ] if team_filter else self.free_kicks_data[
            self.free_kicks_data['zone'].isin([1, 6, 7, 11])     # ← ZONAS LATERALES
        ]
        
        # Diccionario para acumular estadísticas por jugador
        ...
        # Ordenar por número de lanzamientos (criterio principal)
    ...
    def create_faltas_ranking(self, ax, team_filter):
        ...
        # Título
        ax.text(0.5, 1.00, 'MEJORES LANZADORES\n FALTAS LATERALES', fontsize=10, fontweight='bold',
        ...
        for i, player_data in enumerate(top_players):
            # ✅ CAMBIAR: Reducir espaciado entre jugadores de 0.18 a 0.14
            y_pos = 0.78 - (i * 0.14)  # ← CAMBIAR: menos espaciado y posición inicial más baja
            player_name = player_data['player_name']
            lanzamientos = player_data['lanzamientos']
            remates = player_data['remates']
            efectividad = player_data['efectividad']
            
            # ✅ CAMBIAR: Reducir altura del rectángulo de fondo de 0.16 a 0.12
            rect_bg = patches.FancyBboxPatch((0.02, y_pos - 0.06), 0.96, 0.12,  # ← CAMBIAR altura
            ...
            # ✅ CAMBIAR: Reducir tamaño de la foto
            player_photo = self.get_player_photo_with_team_filter(player_name, photos_data, team_filter)
            if player_photo is not None:
                # ← CAMBIAR: altura de foto de 0.20 a 0.15 y ajustar posición Y
                photo_ax = ax.inset_axes([0.12, y_pos - 0.065, 0.32, 0.15])
            ...
            if line2 is None:
                ...
                ax.text(0.40, y_pos + 0.04, line1, fontsize=10, fontweight='bold', 
                        va='center', ha='left', color='#2c3e50')
            else:
                # ✅ CAMBIAR: Ajustar posiciones de las líneas de texto
                ax.text(0.40, y_pos + 0.045, line1, fontsize=9, fontweight='bold', 
                        va='center', ha='left', color='#2c3e50')
                ax.text(0.40, y_pos + 0.015, line2, fontsize=9, fontweight='bold', 
                        va='center', ha='left', color='#2c3e50')
            
            # ✅ CAMBIAR: Ajustar posiciones de las estadísticas
            ax.text(0.45, y_pos - 0.03, f'{lanzamientos} lanz. | {remates} rem.', 
                    fontsize=8, va='center', ha='left', color='#7f8c8d')
            ax.text(0.45, y_pos - 0.05, f'Efect: {efectividad:.1f}%', 
            ...
            star_text = '★' * stars
            ax.text(0.75, y_pos, star_text, fontsize=12, va='center', ha='center',
                    color='#f39c12')
    ...
    def point_in_zone(self, x, y, zone_coords):
        """Determina si un punto está dentro de una zona rectangular"""
        try:
            x, y = float(x), float(y)
    ...
    def extract_free_kicks_data(self, team_filter=None):
        ...
        # Quitamos el filtro por la columna 'Zone' y usamos nuestra 'start_zone' calculada.
        # Las zonas laterales son 1, 6 (banda derecha cercana y lejana) y 7, 11 (banda izquierda).
        free_kicks_directos = df_sorted[
            (df_sorted.get('Free kick taken', '') == 'Sí') &
            (df_sorted['x'] > 60) &
            (df_sorted['start_zone'].isin([1, 6, 7, 11])) &  # <-- ESTE ES EL NUEVO FILTRO
            (df_sorted['x'].notna()) &
            (df_sorted['y'].notna())
        ]
        ...
        for _, free_kick in free_kicks_directos_unique.iterrows():
            try:
                ...
                # Tomamos la zona de la columna que ya hemos calculado.
                ...
                free_kick_data = {
                    ...
                    'zone': zone,  # <-- Usamos la zona pre-calculada
                    'outcome': free_kick['outcome'],
                    'Event Name': free_kick['Event Name']
                }
                
                # Analizar la secuencia posterior
    ...
    def prepare_free_kicks_data(self, team_filter=None):
        """Prepara datos de faltas para visualización"""
        if self.free_kicks_data.empty:
            print("❌ No hay datos de faltas para preparar")
            return {}

        # Estructura para organizar los datos por zona y posición
        zone_data = {}
        ...
        for _, free_kick in team_free_kicks.iterrows():
            ...
            point_data = {
                'x': free_kick['x'],
                'y': free_kick['y'],
                'pass_end_x': float(free_kick.get('Pass End X', 0)),  # ← AÑADIR
                'pass_end_y': float(free_kick.get('Pass End Y', 0)),  # ← AÑADIR
    ...
    def get_top_lanzadores_por_zona(self, zona_lanzamiento, team_filter=None):
        """Obtiene el top 3 de jugadores con más lanzamientos en una zona específica"""
        
        # Filtrar faltas de la zona de lanzamiento específica
        if self.free_kicks_data.empty:
            return []
        ...
        # Ordenar por número de lanzamientos y tomar top 3
    ...
    def create_mapa_calor_zona_lanzamiento(self, ax, zona_target, titulo):
        """Crea mapa de calor para una zona de lanzamiento específica con ranking de jugadores"""
        ...
        # Obtener datos de caídas para esta zona de lanzamiento
        zonas_caida = self.get_caidas_por_zona(zona_target)
        
        # Obtener el máximo de caídas para normalizar colores
        ...
        orden_dibujo = ['zona_1', 'zona_2', 'zona_3', 'zona_4', 'zona_5', 'zona_6', 
                        'zona_7', 'zona_8', 'zona_9', 'zona_10', 'zona_11']
        ...
        for zona in orden_dibujo:
            if zona in zona_coords_matplotlib:
                ...
                if zona == f'zona_{zona_target}':
                    ...
                    # Otras zonas con intensidad según caídas
                    if max_caidas > 0:
                ...
                # Texto con número de caídas
                center_y, center_x = y_min + width/2, x_min + height/2
                
                if zona == f'zona_{zona_target}':
                    # En zona de lanzamiento, mostrar balón
                    ...
                    # Solo mostrar número si hay caídas
                    if count_caidas > 0:
    ...
    def create_free_kicks_visualization(self, figsize=(11.69, 8.27), team_filter=None):
        """Crear visualizaciÃ³n de faltas ofensivas laterales con mapas de calor y ranking"""
        plt.rcParams.update({
        ...
        fig.suptitle('ANÁLISIS DE FALTAS OFENSIVAS LATERALES',
        ...
        if team_filter and (team_logo := self.load_team_logo(team_filter)) is not None:
            ax_team = fig.add_axes([0.90, 0.90, 0.08, 0.09])
            ax_team.imshow(team_logo, aspect='auto')
            ax_team.axis('off')
            
        if (villarreal_logo := self.load_villarreal_logo()) is not None:
            ax_villarreal = fig.add_axes([0.87, 0.90, 0.08, 0.09])
            ax_villarreal.imshow(villarreal_logo, aspect='auto')
            ax_villarreal.axis('off')

        # --- AQUÃ ESTÃ LA LÃ“GICA DE LA DISPOSICIÃ“N ---
        # Se crea una rejilla de 2 filas y 5 columnas para colocar los 4 mapas y el ranking
        gs = GridSpec(2, 5, figure=fig,
            left=0.05, right=0.95,
            bottom=0.14, top=0.90,
            wspace=0.08, hspace=0.15)

        # FILA SUPERIOR: Zona 1 (columnas 0-1) + Ranking (columna 2) + Zona 6 (columnas 3-4)
        ax_zona1 = fig.add_subplot(gs[0, 0:2])   # Arriba, Izquierda
        ax_ranking = fig.add_subplot(gs[:, 2])   # Ranking en columna central, ambas filas
        ax_zona6 = fig.add_subplot(gs[0, 3:5])   # Arriba, Derecha

        # FILA INFERIOR: Zona 7 (columnas 0-1) y Zona 11 (columnas 3-4)
        ax_zona7 = fig.add_subplot(gs[1, 0:2])   # Abajo, Izquierda
        ax_zona11 = fig.add_subplot(gs[1, 3:5])  # Abajo, Derecha

        # Se asigna cada grÃ¡fico a su zona correspondiente
        campos_config = [
            (ax_zona1, 1, 'LATERAL IZQ. CERCANA'),
            (ax_zona6, 6, 'LATERAL DER. CERCANA'),
            (ax_zona7, 7, 'LATERAL IZQ. LEJANA'),
            (ax_zona11, 11, 'LATERAL DER. LEJANA')
        ]
